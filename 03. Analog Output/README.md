...
* **Hiệu suất năng lượng cao (Ít tỏa nhiệt):**
  * *Xuất Analog bằng DAC/Tuyến tính:* Linh kiện phải gánh phần điện áp dư thừa và tiêu tán thành **nhiệt năng**, gây lãng phí năng lượng.
  * *Xuất bằng PWM (Đóng/Cắt):* Transistor công suất chỉ hoạt động ở 2 trạng thái BẬT hoàn toàn (ON) hoặc TẮT hoàn toàn (OFF). Công suất hao phí $P = I^2 \cdot R \approx 0$, đạt hiệu suất chuyển đổi năng lượng rất cao ($> 90\%$).
...
* **Điều khiển góc quay Servo:** Gửi xung PWM có độ rộng chuẩn ($1\text{ms} - 2\text{ms}$) để điều khiển chính xác vị trí góc quay ($0^\circ - 180^\circ$).
...
* **Độ phân giải (Resolution):** Trong MicroPython, thang đo Duty Cycle chuẩn cho PWM sử dụng dải 10-bit (`0` đến `1023`). Do đó, giá trị đọc về từ ADC 12-bit (`0` đến `4095`) cần được quy đổi (chia cho 4 hoặc dịch bit `>> 2`) để tương thích hoàn hảo với tín hiệu PWM xuất ra.
